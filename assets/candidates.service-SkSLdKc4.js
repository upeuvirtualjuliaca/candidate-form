...
`;async function n(n=1,r=10,i,a){let o=(n-1)*r,s=o+r-1,c=e.from(`candidates`).select(t,{count:`exact`}).is(`deleted_at`,null).order(`created_at`,{ascending:!1});if(i&&(c=c.eq(`status`,i)),a?.trim()){let t=`%${a.trim()}%`,[{data:n},{data:r}]=await Promise.all([e.from(`students`).select(`id`).or(`full_name.ilike.${t},dni.ilike.${t},student_code.ilike.${t}`),e.from(`teachers`).select(`id`).or(`full_name.ilike.${t},dni.ilike.${t}`)]),i=(n??[]).map(e=>e.id),o=(r??[]).map(e=>e.id);if(i.length===0&&o.length===0)return{data:[],count:0};let s=[];i.length>0&&s.push(`student_id.in.(${i.join(`,`)})`),o.length>0&&s.push(`teacher_id.in.(${o.join(`,`)})`),c=c.or(s.join(`,`))}c=c.range(o,s);let{data:l,count:u,error:d}=await c;if(d)throw d;return{data:l??[],count:u??0}}async function r(t){let{data:n,error:r}=await e.from(`candidates`).select(`
