...
<template>
  <header
    class="sticky top-0 z-10 flex items-center gap-1 lg:gap-4 px-2 md:px-4 lg:px-6 h-20 lg:h-16 bg-white border-b border-gray-100 shadow-sm">
    ...
    <div class="flex items-center gap-3">
      ...
      <div ref="menuRef" class="relative">
        <button type="button" @click="toggle"
          ...
          <!-- Avatar -->
          <div
            class="w-8 h-8 rounded-full bg-[#04395a] flex items-center justify-center text-white text-xs font-bold shrink-0">
            {{ initials }}
          </div>
          <div class="hidden sm:block leading-tight text-left">
            <p
              class="text-xs font-semibold text-gray-700 group-hover:text-[#04395a] transition-colors truncate max-w-[140px]">
              {{ displayName }}
            </p>
        ...
        <Transition enter-active-class="transition duration-150 ease-out"
          ...
          <div v-if="open"
            ...
            <div class="px-4 py-4 border-b border-gray-100">
              <div class="flex items-center gap-3">
                <div
                  class="w-10 h-10 rounded-full bg-[#04395a] flex items-center justify-center text-white text-sm font-bold shrink-0">
                  {{ initials }}
                </div>
                <div class="min-w-0">
                  <p class="text-sm font-semibold text-gray-800 truncate">{{ displayName }}</p>
                  <span class="inline-block mt-0.5 text-[10px] font-semibold px-2 py-0.5 rounded-full" :class="{
            ...
            <div v-if="campaigns.length > 0" class="px-3 py-3 border-b border-gray-100">
              ...
              <!-- Trigger button -->
              <button type="button" @click="toggleCampaignDrop"
                ...
                <!-- Campaign name -->
                <span
                  class="flex-1 text-xs font-semibold text-gray-700 truncate group-hover:text-[#04395a] transition-colors">
              ...
              <Transition enter-active-class="transition duration-150 ease-out"
                ...
                <div v-if="campaignDropOpen"
                  class="mt-1.5 rounded-xl border border-gray-200 bg-white shadow-lg overflow-hidden" @click.stop>
                  <!-- Search input -->
                  <div class="p-2 border-b border-gray-100 bg-gray-50">
                    <div class="relative flex items-center">
                      <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2"
                        stroke="currentColor" class="absolute left-2.5 w-3.5 h-3.5 text-gray-400 pointer-events-none">
                        <path stroke-linecap="round" stroke-linejoin="round"
                          d="m21 21-5.197-5.197m0 0A7.5 7.5 0 1 0 5.196 5.196a7.5 7.5 0 0 0 10.607 10.607Z" />
                      </svg>
                      <input ref="campaignInputRef" v-model="campaignSearch" type="text" placeholder="Buscar campaña…"
                  ...
                  <ul class="max-h-44 overflow-y-auto divide-y divide-gray-50">
                    ...
                    <li v-for="c in filteredCampaigns" :key="c.id">
                      <button type="button" @click.stop="selectCampaign(c.id)"
                        ...
                        <!-- Status badge -->
                        <span class="shrink-0 text-[9px] font-bold px-1.5 py-0.5 rounded-full leading-none"
                          :class="statusBadgeClass(c)">
                          {{ statusLabelText(c) }}
                        </span>

                        <!-- Selected check -->
                        <svg v-if="c.id === campaignStore.selected?.id" xmlns="http://www.w3.org/2000/svg" fill="none"
                          viewBox="0 0 24 24" stroke-width="2.5" stroke="#04395a" class="w-3.5 h-3.5 shrink-0">
