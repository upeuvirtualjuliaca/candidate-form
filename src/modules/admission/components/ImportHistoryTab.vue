...
import { fetchImportLogs, type ImportLog, type ImportStatus } from '../services/importStudents.service'

// ── Props ──────────────────────────────────────────────────────────────────
// Parent passes a counter that increments after each new import so we refetch
...
async function load() {
  ...
  error.value   = ''
  try {
    const res    = await fetchImportLogs(page.value, pageSize)
    logs.value   = res.data
...
<template>
  <div class="space-y-4">
    ...
    <div v-if="error && !loading" class="bg-red-50 border border-red-200 rounded-xl p-4 flex items-center gap-3">
      ...
      <button type="button" @click="load" class="ml-auto text-xs font-medium text-red-700 underline underline-offset-2">
        Reintentar
      </button>
    </div>
    ...
    <template v-else>
        ...
        >
          ...
          <div class="hidden sm:grid grid-cols-[2fr_1fr_1fr_1fr_1fr_1fr_1.5fr] gap-3 items-center px-4 py-3.5">
            ...
            <div class="flex items-center gap-2.5 min-w-0">
              ...
              <span class="text-sm font-medium text-gray-700 truncate" :title="log.file_name">
                {{ shortName(log.file_name) }}
              </span>
            </div>

            <!-- Numeric cols -->
            <span class="text-sm text-gray-600 text-right tabular-nums">
              {{ log.total_records.toLocaleString() }}
            </span>
            <span class="text-sm font-medium text-emerald-600 text-right tabular-nums">
              {{ log.inserted_records.toLocaleString() }}
            </span>
            <span class="text-sm text-amber-600 text-right tabular-nums">
              {{ log.duplicate_records.toLocaleString() }}
            </span>
            <span
              class="text-sm text-right tabular-nums"
              :class="log.error_records > 0 ? 'text-red-600 font-medium' : 'text-gray-400'"
            >
              {{ log.error_records.toLocaleString() }}
            </span>

            <!-- Badge -->
            <span
              :class="['inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-semibold w-fit', badgeClasses[log.status]]"
            >
          ...
          <div class="sm:hidden p-4 space-y-3">
            <div class="flex items-start justify-between gap-2">
              ...
              <span
                :class="['inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-semibold shrink-0', badgeClasses[log.status]]"
              >
                <span :class="['w-1.5 h-1.5 rounded-full', badgeDot[log.status]]" />
                {{ badgeLabels[log.status] }}
              </span>
            ...
            <div class="grid grid-cols-4 gap-2 text-center">
              ...
              <div class="bg-white rounded-lg p-2">
                <p class="text-sm font-bold tabular-nums" :class="log.error_records > 0 ? 'text-red-600' : 'text-gray-400'">
                  {{ log.error_records.toLocaleString() }}
                </p>
      ...
      >
        <p class="text-xs text-gray-400">
          {{ from }}–{{ to }} de {{ totalCount.toLocaleString() }} registros
        </p>
