...
import { parseExcelFile, type ParsedStudent } from '../utils/excelStudents.parser'
import { importStudents, type ImportProgress, type ImportResult } from '../services/importStudents.service'
...
const canImport = computed(() =>
  stage.value === 'idle' && selectedFile.value !== null && parsedStudents.value.length > 0,
)
...
async function handleFile(file: File) {
  ...
  try {
    ...
    if (parseResult.missingColumns.length > 0) {
      missingCols.value  = parseResult.missingColumns
      stage.value        = 'error'
    ...
    parsedStudents.value = parseResult.students
    parseSkipped.value   = parseResult.skippedRows
    stage.value          = 'idle'
  } catch (err) {
...
async function startImport() {
  ...
  progress.value = { processed: 0, total: parsedStudents.value.length, percent: 0 }

  try {
    const importResult = await importStudents(parsedStudents.value, (p) => {
      progress.value = p
    })
...
<template>
  <div class="space-y-6 max-w-3xl mx-auto">
    ...
    <!-- Page heading -->
    <div>
      <h2 class="text-2xl font-bold text-[#04395a]">Importar Estudiantes</h2>
      <p class="text-sm text-gray-500 mt-0.5">
        Carga un archivo Excel (.xlsx / .xls) para registrar estudiantes en el sistema.
      </p>
    </div>
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
      <div class="px-6 py-4 border-b border-gray-100 flex items-center gap-2">
        <span class="w-6 h-6 rounded-full bg-[#04395a] text-white text-xs font-bold flex items-center justify-center shrink-0">1</span>
        <span class="text-sm font-semibold text-gray-700">Seleccionar archivo</span>
      </div>
      ...
      <div class="p-6">
        ...
        <!-- Parsing spinner -->
        <div v-else-if="stage === 'parsing'" class="flex items-center gap-3 py-4">
          <svg class="w-5 h-5 text-[#068ab8] animate-spin" fill="none" viewBox="0 0 24 24">
            <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4" />
            <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8v4a4 4 0 00-4 4H4z" />
          </svg>
          <span class="text-sm text-gray-600">Leyendo archivo…</span>
        </div>

        <!-- File selected preview -->
        <div
          v-else-if="selectedFile && stage !== 'importing'"
        ...
        >
          ...
          <div class="flex-1 min-w-0">
            ...
            <p class="text-xs text-gray-400 mt-0.5">
              {{ parsedStudents.length.toLocaleString() }} registros listos
    ...
    <!-- ── Step 2: Import ──────────────────────────────────────────────── -->
    <div
      v-if="parsedStudents.length > 0 || stage === 'importing' || stage === 'done'"
      class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden"
    >
      <div class="px-6 py-4 border-b border-gray-100 flex items-center gap-2">
        <span class="w-6 h-6 rounded-full bg-[#04395a] text-white text-xs font-bold flex items-center justify-center shrink-0">2</span>
        <span class="text-sm font-semibold text-gray-700">Importar a la base de datos</span>
      </div>
      ...
      <div class="p-6 space-y-5">
        ...
        <div v-if="stage === 'idle'" class="grid grid-cols-2 sm:grid-cols-3 gap-3">
          <div class="bg-[#04395a]/5 rounded-xl p-4 text-center">
            <p class="text-2xl font-bold text-[#04395a]">{{ parsedStudents.length.toLocaleString() }}</p>
            <p class="text-xs text-gray-500 mt-0.5">Registros a importar</p>
          </div>
          <div class="bg-amber-50 rounded-xl p-4 text-center">
            <p class="text-2xl font-bold text-amber-600">{{ parseSkipped }}</p>
            <p class="text-xs text-gray-500 mt-0.5">Filas vacías omitidas</p>
          </div>
        ...
        <div v-if="stage === 'importing'" class="space-y-3">
          ...
          <p class="text-xs text-gray-400 text-right">
            {{ progress.processed.toLocaleString() }} / {{ progress.total.toLocaleString() }} registros
          </p>
        </div>
        ...
        <button
          ...
          :disabled="!canImport"
          @click="startImport"
          class="w-full sm:w-auto flex items-center justify-center gap-2 px-6 py-3 rounded-xl text-sm font-semibold text-white bg-[#04395a] hover:bg-[#068ab8] transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
        >
      ...
      >
        ...
        <div class="px-6 py-4 bg-emerald-50 border-b border-emerald-100 flex items-center gap-3">
          ...
          <div>
            ...
            <p class="text-xs text-emerald-600">Los registros han sido procesados exitosamente.</p>
          </div>
        </div>
        ...
        <div class="p-6 grid grid-cols-2 sm:grid-cols-4 gap-4">
          <div class="text-center p-4 bg-gray-50 rounded-xl">
            <p class="text-2xl font-bold text-gray-700">{{ result.total.toLocaleString() }}</p>
            <p class="text-xs text-gray-500 mt-1">Total leídos</p>
          </div>
          <div class="text-center p-4 bg-emerald-50 rounded-xl">
            <p class="text-2xl font-bold text-emerald-600">{{ result.inserted.toLocaleString() }}</p>
            <p class="text-xs text-gray-500 mt-1">Insertados</p>
          </div>
          <div class="text-center p-4 bg-amber-50 rounded-xl">
            <p class="text-2xl font-bold text-amber-600">{{ result.duplicates.toLocaleString() }}</p>
            <p class="text-xs text-gray-500 mt-1">Actualizados</p>
          </div>
          <div class="text-center p-4 rounded-xl" :class="result.errors > 0 ? 'bg-red-50' : 'bg-gray-50'">
            <p class="text-2xl font-bold" :class="result.errors > 0 ? 'text-red-600' : 'text-gray-400'">
              {{ result.errors }}
            </p>
            <p class="text-xs text-gray-500 mt-1">Errores</p>
          </div>
        </div>
        ...
        <div class="px-6 pb-6 flex flex-wrap gap-3">
          ...
          <RouterLink
            to="/students"
            class="flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-medium text-[#04395a] bg-[#04395a]/8 hover:bg-[#04395a]/15 transition-colors"
          >
            Ver estudiantes
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.8" stroke="currentColor" class="w-4 h-4">
    ...
    <Transition name="slide-up">
      <div
        v-if="stage === 'error'"
        class="bg-red-50 border border-red-200 rounded-2xl p-5 flex gap-4"
      >
        ...
        <div class="flex-1 min-w-0">
          <p class="text-sm font-semibold text-red-800">Error en la importación</p>
          <p class="text-sm text-red-600 mt-1">{{ errorMessage }}</p>
          <ul v-if="missingCols.length" class="mt-2 space-y-1">
            <li
              v-for="col in missingCols"
              :key="col"
              class="text-xs text-red-500 font-mono bg-red-100 inline-block px-2 py-0.5 rounded mr-1 mb-1"
            >
              {{ col }}
            </li>
          </ul>
          <button
            type="button"
            @click="clearFile"
            class="mt-3 text-xs font-medium text-red-700 underline underline-offset-2 hover:text-red-900"
          >
            Intentar de nuevo
          </button>
        </div>
