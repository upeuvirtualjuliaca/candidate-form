...
import {
  ...
  type ImportProgress,
  type ImportResult,
} from '../services/importStudents.service'
...
// Increment after each successful import so ImportHistoryTab refetches
...
const isLoading = computed(() => stage.value === 'parsing' || stage.value === 'importing')
...
async function handleFile(file: File) {
  ...
  try {
    ...
    parsedStudents.value = parsed.students
...
async function startImport() {
  ...
  try {
    ...
    // Persist log (fire-and-forget — errors are swallowed inside saveImportLog)
    await saveImportLog(fileName, importResult)
...
<template>
  <div class="space-y-6">
    <!-- Heading -->
    <div>
      <h2 class="text-2xl font-bold text-[#04395a]">Estudiantes</h2>
      <p class="text-sm text-gray-500 mt-0.5">
        Gestión de importaciones y registro de estudiantes.
      </p>
    </div>
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
      ...
      <div class="px-6 pb-6">
        ...
        <div v-show="activeTab === 'import'" class="mt-2 space-y-5">
          ...
          >
            <div
              class="w-10 h-10 rounded-xl bg-emerald-50 flex items-center justify-center shrink-0"
            >
              <svg
                xmlns="http://www.w3.org/2000/svg"
                fill="none"
            ...
            <div class="flex-1 min-w-0">
              ...
              <p class="text-xs text-gray-400 mt-0.5">
                ...
                <span v-if="parseSkipped > 0" class="text-amber-500">
                  · {{ parseSkipped }} filas omitidas</span
                >
              </p>
            </div>
            <button
          ...
          >
            ...
            <div v-if="stage === 'idle'" class="grid grid-cols-2 sm:grid-cols-3 gap-3">
              <div class="bg-[#04395a]/5 rounded-xl p-4 text-center">
                <p class="text-2xl font-bold text-[#04395a]">
                  {{ parsedStudents.length.toLocaleString() }}
                </p>
            ...
            <div v-if="stage === 'importing'" class="space-y-3">
              ...
              <p class="text-xs text-gray-400 text-right">
                ...
                {{ progress.total.toLocaleString() }} registros
              </p>
            </div>
          ...
          <Transition name="slide-up">
            ...
            >
              <div
                class="px-5 py-4 bg-emerald-50 border-b border-emerald-100 flex items-center gap-3"
              >
                <div
                  class="w-7 h-7 rounded-full bg-emerald-500 flex items-center justify-center shrink-0"
                >
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    fill="none"
              ...
              <div class="p-5 grid grid-cols-2 sm:grid-cols-4 gap-3">
                ...
                <div
                  ...
                  :class="result.errors > 0 ? 'bg-red-50' : 'bg-gray-50'"
                >
                  <p
                    class="text-xl font-bold"
                    :class="result.errors > 0 ? 'text-red-600' : 'text-gray-400'"
                  >
                    {{ result.errors }}
                  </p>
              ...
              <div class="px-5 pb-5 flex flex-wrap gap-2">
                ...
                <RouterLink
                  to="/students"
                  class="flex items-center gap-1.5 px-4 py-2 rounded-xl text-sm font-medium text-gray-500 hover:text-gray-700 hover:bg-gray-100 transition-colors"
                >
                  Ver estudiantes
                </RouterLink>
              </div>
          ...
          <Transition name="slide-up">
            ...
            >
              <div
                class="w-7 h-7 rounded-full bg-red-100 flex items-center justify-center shrink-0 mt-0.5"
              >
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  fill="none"
              ...
              <div class="flex-1 min-w-0">
                ...
                <div v-if="missingCols.length" class="flex flex-wrap gap-1 mt-2">
                  <span
                    ...
                    :key="col"
                    class="text-xs text-red-500 font-mono bg-red-100 px-2 py-0.5 rounded"
                    >{{ col }}</span
                  >
                </div>
                <button
                  type="button"
                  @click="clearFile"
                  class="mt-2 text-xs font-medium text-red-700 underline underline-offset-2"
                >
                  Intentar de nuevo
                </button>
              </div>
        ...
        <div v-show="activeTab === 'history'" class="mt-2">
          <ImportHistoryTab :refresh-trigger="historyRefresh" />
        </div>
      </div>
    </div>
  </div>
